...
export async function prepareRelease() {
	...
	async function build(os: "linux" | "darwin" | "windows") {
		await exe(
			...
			[
				...
				"-o",
				`./dist/ssh_add_with_pass_${os}_amd64`,
				".",
			],
			{
				env: {
		...
		await Promise.all([
			(async () => {
				const a = archiver(os === "windows" ? "zip" : "tar", {
					gzip: true,
				});
				...
				a.append(
					fs.createReadStream(`./dist/ssh_add_with_pass_${os}_amd64`),
					{
						name:
							os === "windows"
								? "ssh_add_with_pass.exe"
								: "ssh_add_with_pass",
					}
				);
				a.pipe(
					fs.createWriteStream(
						`./dist/github-downloads/ssh_add_with_pass_${os}_amd64.${
				...
				await a.finalize();
				log.info(
					`packaged ./dist/github-downloads/ssh_add_with_pass_${os}_amd64.${
						os === "windows" ? "zip" : "tar.gz"
					}`
				);
			})(),
			(async () => {
				if (os === "linux") {
					await exe(log.getChildLogger({ prefix: ["nfpm"] }), [
						"docker",
						"pull",
						`goreleaser/nfpm:v${config.nfpmVersion}`,
					]);

					const nfpm = (type: "rpm" | "deb" | "apk") =>
						exe(
							...
							[
								...
								"pkg",
								"--target",
								`./dist/github-downloads/ssh_add_with_pass_linux_amd64.${type}`,
							]
						);

					await Promise.all([nfpm("apk"), nfpm("rpm"), nfpm("deb")]);
				}
			})(),
		]);
	}

	await Promise.all([build("linux"), build("darwin"), build("windows")]);
	...
	brew = brew.replace(
		/\$\{HASH\}/g,
		await hasha.fromFile(
			"./dist/github-downloads/ssh_add_with_pass_darwin_amd64.tar.gz",
			{ algorithm: "sha256" }
		)
	);
	await fs.promises.writeFile(
		"./dist/homebrew-tap/ssh_add_with_pass.rb",
		brew,
		"utf8"
	);
	...
	scoop = scoop.replace(
		/\$\{HASH\}/g,
		await hasha.fromFile(
			"./dist/github-downloads/ssh_add_with_pass_windows_amd64.zip",
			{ algorithm: "sha256" }
		)
	);
	await fs.promises.writeFile(
		"./dist/scoop-bucket/ssh_add_with_pass.json",
		scoop,
		"utf8"
	);
	...
	await Promise.all([
		(async () => {
			...
			await git.clone({
				...
				onMessage: (msg) => {
					brewLog.info(msg.trim());
				},
			});
			await unlinkIfExists(
				brewLog,
				"./dist/homebrew-tap/repo/Formula/ssh_add_with_pass.rb"
			);
			...
			brewLog.info(
				"copied ./dist/homebrew-tap/ssh_add_with_pass.rb => ./dist/homebrew-tap/repo/Formula/ssh_add_with_pass.rb"
			);
			await git.add({
				fs,
				dir: "./dist/homebrew-tap/repo",
				filepath: "Formula/ssh_add_with_pass.rb",
			});
			brewLog.info(
				"git add ./dist/homebrew-tap/repo/Formula/ssh_add_with_pass.rb"
			);
			await git.commit({
				fs,
				dir: "./dist/homebrew-tap/repo",
				message: `chore(ssh_add_with_pass): release new version ${config.versionNo}`,
				author: {
			...
			brewLog.info(
				`git commit -m "chore(ssh_add_with_pass): release new version ${config.versionNo}"`
			);
		...
		(async () => {
			...
			await unlinkIfExists(
				scoopLog,
				"./dist/scoop-bucket/repo/ssh_add_with_pass.json"
			);
			await fs.promises.copyFile(
				"./dist/scoop-bucket/ssh_add_with_pass.json",
				"./dist/scoop-bucket/repo/ssh_add_with_pass.json"
			...
			scoopLog.info(
				"copied ./dist/scoop-bucket/ssh_add_with_pass.json => ./dist/scoop-bucket/repo/ssh_add_with_pass.json"
			);
			await git.add({
				fs,
				dir: "./dist/scoop-bucket/repo",
				filepath: "ssh_add_with_pass.json",
			});
			scoopLog.info(
				"git add ./dist/scoop-bucket/repo/ssh_add_with_pass.json"
			);
			await git.commit({
				fs,
				dir: "./dist/scoop-bucket/repo",
